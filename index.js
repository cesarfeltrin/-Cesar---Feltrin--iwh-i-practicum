...
app.post("/update-cobj", async (req, res) => {
  ...
  const especie = Array.isArray(req.body.especie) ? req.body.especie[0] : req.body.especie;
  const idade = Array.isArray(req.body.idade) ? req.body.idade[0] : req.body.idade;
...
app.get("/", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://api.hubapi.com/crm/v3/objects/${CUSTOM_OBJECT}?properties=nome,especie,idade`,
      {
        headers: {
          Authorization: `Bearer ${HUBSPOT_TOKEN}`,
        },
